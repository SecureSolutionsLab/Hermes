...
public class ThroughputLatencyClient {
    ...
    public static void main(String[] args) throws IOException {
        ...
        LatencyClient[] latencyClients = new LatencyClient[numThreads];
        ...
        for(int i=0; i<numThreads; i++) {
            ...
            latencyClients[i] = new LatencyClient(initId+i,numberOfOps,requestSize,interval,readOnly, verbose, s);
        }

        ExecutorService exec = Executors.newFixedThreadPool(latencyClients.length);
        Collection<Future<?>> tasks = new LinkedList<>();
        
        for (LatencyClient c : latencyClients) {
    ...
    static class LatencyClient extends Thread {
        ...
        public LatencyClient(int id, int numberOfOps, int requestSize, int interval, boolean readOnly, boolean verbose, int sign) {
            super("Client "+id);
