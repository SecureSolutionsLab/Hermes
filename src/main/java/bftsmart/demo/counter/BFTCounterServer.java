...
public final class BFTCounterServer extends DefaultRecoverable {

    private int counter = 0;
    private int iterations = 0;

    public BFTCounterServer(int id) {
    ...
    public byte[][] appExecuteBatch(byte[][] commands, MessageContext[] msgCtxs, boolean fromConsensus) {
        ...
        for (byte[] command : commands) {
            replies[index] = appExecuteOrdered(command, msgCtxs[index]);
            index++;
        }
        return replies;
    }
    ...
    public static void main(String[] args){
        System.out.println("[BFT-SMaRt] Starting CounterServer");
        if(args.length < 1) {
            System.out.println("Use: java CounterServer <processId>");
            System.exit(-1);
        }
        new BFTCounterServer(Integer.parseInt(args[0]));
    }
